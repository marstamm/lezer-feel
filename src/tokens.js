...
export class BaseContext {
  ...
  constructor(value = {}) {
    ...
    if (!value || typeof value !== 'object') {
      this.value = {};
    } else if (value instanceof BaseContext) {
   ...
   *
   * @param {String} key
   * @returns {BaseContext|ValueProducer|null}
   */
  get(key) {
    ...
    return new BaseContext(this.value[key]);
   ...
   * @param {String} key
   * @param {any} value
   * @returns {BaseContext} new context with the given key added
   */
  set(key, value) {
 ...
 */
export function trackVariables(context = {}, ContextClass = BaseContext) {

  const start = Variables.of({
    context: new ContextClass(context),
  });
    ...
    reduce(variables, term, stack, input) {
      ...
      if (prefixedStart) {
        ...
        if (term === pathExpressionStart) {
          newContext = new ContextClass(lastChild?.computedValue());
        }

        if (term === filterExpressionStart) {
          newContext = ContextClass.merge(
            currentContext,
      ...
      if (term === ContextEntry) {
        ...
        return wrap(variables, 'ContextEntry', code).assign(
          {
            value:
                new ContextClass(variables.value)
